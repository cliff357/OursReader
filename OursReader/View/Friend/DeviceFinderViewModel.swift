...
class DeviceFinderViewModel: NSObject, ObservableObject {
    ...
    func peerJoin(peerId: MCPeerID) {
        guard let first = peers.first(where: { $0.peerId.displayName == peerId.displayName }) else {
            return
        }
        
        // Avoid adding duplicate peers
        if !joinedPeer.contains(where: { $0.peerId.displayName == peerId.displayName }) {
    ...
    func printSessionInfo() {
        for peer in session.connectedPeers {
            print("session peer: \(peer.displayName)")
        }
        
        for peer in self.joinedPeer {
            print("joined peer: \(peer.peerId.displayName)")
        }
    }
    #endif
    ...
    private func connect() {
        ...
        if !joinedPeer.contains(where: { $0.peerId.displayName == selectedPeer.peerId.displayName }) {
...
extension DeviceFinderViewModel: MCNearbyServiceBrowserDelegate {
    func browser(_ browser: MCNearbyServiceBrowser, foundPeer peerID: MCPeerID, withDiscoveryInfo info: [String : String]?) {
        DispatchQueue.main.async {
            guard !self.peers.contains(where: { $0.peerId.displayName == peerID.displayName }) else {
                return
            }
            
            let peer = PeerDevice(peerId: peerID)
...
extension DeviceFinderViewModel {
//    func sendMsg(message: String, peer: PeerDevice) {
